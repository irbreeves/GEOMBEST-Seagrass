function [tempgrid,ii] = stormdeposit(tempgrid,j,t,q_ow,overwashlength,overwashthickness)

% Overwash1 -- Spreads a fixed volume of sand over the backbarrier, from
% the edge of the dune out to a distance determined by the strength of the 
% storm. Thickness of deposition is constant and is a cumulative of all
% storms that occur over the time step.

% David Walters

% Version of 2/14/2013
...
% overwashlength = q_ow / tA; % The extent to which the overwash deposits

while tempvol > 0
    topcellhtT = overwashthickness;% * exp(-(ii-dunelimit) * celldim(1,j)/overwashlength); % The height to which the overwash accretes decays exponentially with the distance from the dunelimit
%     if topcellhtT < 0.05*celldim(3,j)
%         tempvol = 0;
%     end


    % Accrete to the topcellhtT
