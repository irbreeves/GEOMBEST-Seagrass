function findfirstshoreline(t,j)
...
for e = 2:entries %changed from e = 1:entries because of error saying 
    ...
    if strat(j,1).elevation(e,2) >= 0
        upperx = strat(j,2).elevation(e,1);
        lowerx = strat(j,2).elevation(e - 1,1);
        upperz = strat(j,2).elevation(e,2);
        lowerz = strat(j,2).elevation(e - 1,2);        
