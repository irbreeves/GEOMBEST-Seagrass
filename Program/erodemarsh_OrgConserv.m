...
%%% In this version, ALL ORGANIC MATTER IS CONSERVED!
...
%         targete = targetDepth - (highwater - H); 
        ...
        while targete > 0
            ...
            if tempevol > (fillmarsh+fillmud)*celldim(3,j)*celldim(1,j) %if there is enough erosion potential, empty the cell
                ...
                fillkeep = fillkeep + (fillmud + (fillmarsh))*celldim(3,j)*celldim(1,j); % REMOVED 12/12/17: Divide by 2 --> 50% of marsh sed (organic fraction) lost from system   <------------------
...
%                 targete = targetDepth - (highwater - H);
                ...
                if tempevol > fillmarsh*celldim(3,j)*celldim(1,j)
                    ...
                    fillkeep = fillkeep + (fillmarsh)*celldim(3,j)*celldim(1,j); % REMOVED 3/13/18: Divide by 2 --> 50% of marsh sed (organic fraction) lost from system
                    ovol = ovol + (fillmarsh*celldim(3,j)*celldim(1,j));
                    fillmarsh = 0;
...
%         targete = targetDepth - (highwater-H);
        ...
        while targete > 0
            ...
            if tempevol > (fillmarsh+fillmud)*celldim(3,j)*celldim(1,j) %if there is enough erosion potential, empty the cell -- this does not include sand in erosion potential
                fillsand = fillsand + tempgrid(ii,n,1);
                tempgrid(ii,n,1:3) = 0; %empties cell by setting equal to zero
                fillkeep = fillkeep + (fillmud + (fillmarsh))*celldim(3,j)*celldim(1,j); % REMOVED 3/13/18: Divide by 2 --> 50% of marsh sed (organic fraction) lost from system    <------------------
                tempevol = tempevol - (fillmud+fillmarsh)*celldim(3,j)*celldim(1,j);
                n = n + 1; %proceed to cell below
                fillmud = tempgrid(ii,n,3);
                fillmarsh = tempgrid(ii,n,2);
...
%                 targete = targetDepth - (highwater - H);
                ...
                if tempevol > fillmarsh*celldim(3,j)*celldim(1,j)
                    tempgrid(ii,n,2) = 0;
                    tempevol = tempevol - fillmarsh*celldim(3,j)*celldim(1,j);
                    fillkeep = fillkeep + (fillmarsh)*celldim(3,j)*celldim(1,j); % REMOVED 12/12/17: Divide by 2 --> 50% of marsh sed (organic fraction) lost from system    <------------------
                    fillmarsh = 0;
...
% %%% Find Marsh Width after erodemarsh but before growmarsh
% 
% if t > 1
